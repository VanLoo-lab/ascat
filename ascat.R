...
# (iv) expected lengths in segments. Defaults to 50 for low-density data (<200000 probes), 200 otherwise.
# output: a data structure containing:
# 1. Tumor_LogR data matrix
# 2. Tumor_BAF data matrix
...
ascat.aspcf = function(ASCATobj, selectsamples = 1:length(ASCATobj$samples), ascat.gg = NULL, segmentlength = ifelse((dim(ASCATobj$Tumor_LogR)[1])<200000,50,100)) {
  attach(ASCATobj)
